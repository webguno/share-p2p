class P2PFileShare {
    ...
    initializeElements() {
        ...
        if (!this.sendModeBtn || !this.receiveModeBtn || !this.sendMode || !this.receiveMode) {
    ...
    attachEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            if (this.qrScanner && !this.qrScanner.classList.contains('hidden')) {
        ...
        // Ensure scanner elements work properly
        if (this.scanQrBtn) {
            this.scanQrBtn.style.pointerEvents = 'auto';
        }
        if (this.closeScannerBtn) {
            this.closeScannerBtn.style.pointerEvents = 'auto';
        }
    ...
    initializeUIState() {
        // Hide all progress elements initially - with null checks
        if (this.fileInfo) this.fileInfo.classList.add('hidden');
        if (this.shareInfo) this.shareInfo.classList.add('hidden');
        if (this.transferProgress) this.transferProgress.classList.add('hidden');
        if (this.connectionInfo) this.connectionInfo.classList.add('hidden');
        if (this.downloadInfo) this.downloadInfo.classList.add('hidden');
        if (this.downloadProgress) this.downloadProgress.classList.add('hidden');
        if (this.qrScanner) this.qrScanner.classList.add('hidden');
    ...
    async openQRScanner() {
        try {
            if (!this.qrScanner) {
                this.showError('QR Scanner not available');
                return;
            }
